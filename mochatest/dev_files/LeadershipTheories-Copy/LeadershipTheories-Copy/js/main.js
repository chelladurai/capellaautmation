//var testModule = require('./config.js');
...
var callChainCount;
var textSaveTimer;
data1 = require('./config-app.js').DATA_PATH;
var __questionData;
...
var testModule1 = require('./config-app.js');
...
function initVars() {
	...
	serSaveData.url = cmHelpers.getPageURL();
...
function loadQuestions() {
	$.ajax({
		...
		url: "http://devmedia.capella.edu/CourseMedia/ansrsource/CapellaSourceFiles/Leadership%20Theories/data/Leadership%20Theories.json",
		dataType: "json",
		success: function(data){
			console.log("questions.json loaded");
			//quizData = data;
			getQuestionType(JSON.stringify(data.questionGroups));
			//console.log(JSON.stringify(data.questionGroups[0].questions[0].questionType));
			onQuestionLoadComplete();		
	   }

   })
   .fail(function(data, textStatus, error){
	  console.log('questions.json did not load, status: '+textStatus+' , error: '+ error);
   })
}
	...
	if(quizData.useAttemptsScore) {
		$.each(userSaveData.numberofattemptsperquestion,
			function(i,numberOfAttemptsPerQuestion) {
				...
				$('#'+numberOfAttemptsPerQuestion.questionguid).find('.questionPointValue').c( getQuestionPointValue( $('#'+numberOfAttemptsPerQuestion.questionguid).attr('id') ) );
			}
		);
	}
...
function getQuestionType(question) {
	...
	//return question;
	//var _jsonData = JSON.parse(question);
	console.log(question);
	//return _jsonData.questionType;
}
